...
test(
  ...
  asyncTest(async (t) => {
    ...
    t.equal(
      typeof subId,
      'string',
      `got sub id as hex string (${typeof subId})`,
    );
...
test(
  ...
  asyncTest(async (t) => {
    ...
    const wrongContractAddress = (
      await deployLogEchoContract({ tools, from: fromAcccount })
    ).contractAddress;
    ...
    const wrongTopic =
      '0xffffffe106361d4f6cd9098051596d565c1dbf7bc20b4c3acb3aaa4204ffffff';
    const filterParams = {
    ...
    await request({
      method: 'eth_sendTransaction',
      params: [
        { from: fromAcccount, to: wrongContractAddress, data: targetTopic },
      ],
    });
    await tools.trackNextBlock();
    ...
    t.equal(
      matchingResults.transactionHash,
      triggeringTxHash,
